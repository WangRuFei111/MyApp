...
import com.mylibrary.camera2.listener.CaptureListener;
import com.mylibrary.camera2.listener.ClickListener;
import com.mylibrary.camera2.listener.ErrorListener;
import com.mylibrary.camera2.listener.JCameraListener;
import com.mylibrary.camera2.listener.TypeListener;
import com.mylibrary.camera2.state.CameraMachine;
import com.mylibrary.camera2.util.FileUtil;
import com.mylibrary.camera2.util.LogUtil;
import com.mylibrary.camera2.util.ScreenUtils;
import com.mylibrary.camera2.view.CameraView;
import com.mylibrary.camera2.view.FoucsView;
...
public class JCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder.Callback, CameraView {
    ...
    //预览比例
    public int PRRVIEW = 0x0000;//比例
    public static final int PRRVIEW_MAX = 0x1111;
    public static final int PRRVIEW_1_1 = 0x0101;
    public static final int PRRVIEW_3_4 = 0x0304;
    public static final int PRRVIEW_9_16 = 0x0916;
    //   requestLayout();
    ...
    private ImageView mPhoto;             //拍照成功图片
...
//        }
//        LogUtil.i("---", "预览大小：W" + widthSize + "---H:" + heightSize);
//        mVideoView.start();

        float widthSize = mVideoView.getMeasuredWidth();
        float heightSize = mVideoView.getMeasuredHeight();
        LayoutParams videoViewParam=new LayoutParams(LayoutParams.MATCH_PARENT, (int) heightSize);
        switch (PRRVIEW) {
            case PRRVIEW_1_1:
                heightSize = widthSize;
                videoViewParam = new LayoutParams(LayoutParams.MATCH_PARENT, (int) heightSize);
                videoViewParam.topMargin=dip2px(getContext(),150);
                break;
            case PRRVIEW_3_4:
                heightSize = widthSize / 3 * 4;
                videoViewParam = new LayoutParams(LayoutParams.MATCH_PARENT, (int) heightSize);
                videoViewParam.topMargin=dip2px(getContext(),80);
                break;
            case PRRVIEW_9_16:
                heightSize = widthSize / 9 * 16;
                videoViewParam = new LayoutParams(LayoutParams.MATCH_PARENT, (int) heightSize);
                videoViewParam.gravity=Gravity.BOTTOM;
                break;
            case PRRVIEW_MAX:
        ...
        screenProp = heightSize / widthSize;
        mVideoView.start();
        machine.restart(mVideoView.getHolder(), screenProp);
        LogUtil.i("---", "预览大小：W" + widthSize + "---H:" + heightSize);
