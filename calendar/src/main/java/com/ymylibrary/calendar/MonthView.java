...
public class MonthView extends View {
    ...
    private int mSelectDayTodayColor;
    ...
    private List<CalendarBean> listCalendarBean = new ArrayList<>();//画原点的日期
    ...
    private void initAttrs(TypedArray array, int year, int month) {
        if (array != null) {
            mSelectDayColor = array.getColor(R.styleable.MonthCalendarView_month_selected_text_color, Color.parseColor("#000000"));
            mSelectDayTodayColor = array.getColor(R.styleable.MonthCalendarView_month_selected_today_text_color, Color.parseColor("#FFFFFF"));
            ...
            mSelectDayColor = Color.parseColor("#000000");
            mSelectDayTodayColor= Color.parseColor("#FFFFFF");
    ...
    private int[] drawThisMonth(Canvas canvas) {
        ...
        for (int day = 0; day < monthDays; day++) {
            ...
            if (dayString.equals(String.valueOf(mSelDay))) {
                ...
                canvas.drawCircle((startRecX + endRecX) / 2, (startRecY + endRecY) * 100 / 195, mSelectCircleSize, mPaint);
                mWeekRow = row + 1;
            }
            if (dayString.equals(String.valueOf(mSelDay))) {
                selectedPoint[0] = row;
                ...
                //当前日期选中时候颜色变化
                if (mCurrDay == mSelDay && mCurrMonth == mSelMonth && mCurrYear == mSelYear) {
                    mPaint.setColor(mSelectDayTodayColor);
                } else {
        ...
        if (mIsShowLunar) {
            ...
            for (int i = 0; i < 42; i++) {
                ...
                if (selected[0] == row && selected[1] == column) {
                    ...
                    if (mCurrDay == mSelDay && mCurrMonth == mSelMonth && mCurrYear == mSelYear) {
                        mLunarPaint.setColor(mSelectDayTodayColor);
                    }else {
                ...
                int startY = (int) (mRowSize * row + mRowSize * 0.78 - (mLunarPaint.ascent() + mLunarPaint.descent()) / 2);
        ...
        if (mIsShowHint) {
            if (listCalendarBean.size() > 0) {
                ...
                for (int day = 0; day < monthDays; day++) {
                    int col = (day + weekNumber - 1) % 7;
                    int row = (day + weekNumber - 1) / 7;
                    for (CalendarBean calendarBean : listCalendarBean) {
                        if ((calendarBean.getDotday() - 1) == day && calendarBean.getDotyear() == mSelYear && (calendarBean.getDotmonth() - 1) == mSelMonth) {
    ...
    public void addDot(CalendarBean calendarBean) {
        listCalendarBean.add(calendarBean);
        invalidate();
    }
     ...
     * @param list
     */
    public void addDots(List<CalendarBean> list) {
    ...
    public void removeDot(CalendarBean calendarBean) {
        ...
        if (listCalendarBean.size() > 0) {
            for (int i = 0; i < listCalendarBean.size(); i++) {
                if (listCalendarBean.size() <= 0) break;
                if (listCalendarBean.get(i).getDotday() == calendarBean.getDotday() &&
                        listCalendarBean.get(i).getDotmonth() == calendarBean.getDotmonth() &&
                        listCalendarBean.get(i).getDotyear() == calendarBean.getDotyear())
                    listCalendarBean.remove(i);
     ...
     * 删除多个原点
     */
    public void removeDots(List<CalendarBean> list) {
        ...
        for (int i = 0; i < listCalendarBean.size(); i++) {
            ...
            for (CalendarBean calendar : list) {
                if (listCalendarBean.get(i).getDotday() == calendar.getDotday() &&
                        listCalendarBean.get(i).getDotmonth() == calendar.getDotmonth() &&
                        listCalendarBean.get(i).getDotyear() == calendar.getDotyear()) {
                    listInt.add(i);
                }
            }
        ...
        for (int i = 0; i < listInt.size(); i++) {
            listCalendarBean.remove(listInt.get(i) - i);
        }
        invalidate();
    }
