package com.luoye.app.view.clock;
...
public class RemindClockView extends View {

    //使用wrap_content时默认的尺寸
    private final static int DEFAULT_SIZE = 400;

    //刻度线宽度
    private final static int MARK_WIDTH = 3;

    //刻度线长度
    private final static int MARK_LENGTH = 8;

    //刻度线与圆的间隙
    private final static int MARK_GAP = 3;

    //时针宽度
    private final static int HOUR_LINE_WIDTH = 4;

    //分针宽度
    private final static int MINUTE_LINE_WIDTH = 3;

    //秒针宽度
    private final static int SECOND_LINE_WIDTH = 3;
    ...
    //时针的颜色
    private int mHourColor = Color.parseColor("#323D5B");
    //分针的颜色
    private int mMinuteColor = Color.parseColor("#323D5B");
    //秒针的颜色
    private int mSecondColor = Color.parseColor("#323D5B");
    //一刻钟刻度线的颜色
    private int mQuarterMarkColor = Color.parseColor("#323D5B");
    //分钟刻度线的颜色
    private int mMinuteMarkColor = Color.parseColor("#ABB7D6");
    //是否绘制3个指针的圆心
    private boolean isDrawCenterCircle = true;
    //获取时间监听
    private ClockView.OnCurrentTimeListener onCurrentTimeListener;

    public void setOnCurrentTimeListener(ClockView.OnCurrentTimeListener onCurrentTimeListener) {
        this.onCurrentTimeListener = onCurrentTimeListener;
    }

    public RemindClockView(Context context) {
        super(context);
        init();
    }

    public RemindClockView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RemindClockView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ClockView);
        mCircleColor = a.getColor(R.styleable.ClockView_circle_color, Color.parseColor("#FFFFFF"));
        mHourColor = a.getColor(R.styleable.ClockView_hour_color, Color.parseColor("#323D5B"));
        mMinuteColor = a.getColor(R.styleable.ClockView_minute_color, Color.parseColor("#323D5B"));
        mSecondColor = a.getColor(R.styleable.ClockView_second_color, Color.parseColor("#323D5B"));
        mQuarterMarkColor = a.getColor(R.styleable.ClockView_quarter_mark_color, Color.parseColor("#323D5B"));
        mMinuteMarkColor = a.getColor(R.styleable.ClockView_minute_mark_color, Color.parseColor("#ABB7D6"));
        isDrawCenterCircle = a.getBoolean(R.styleable.ClockView_draw_center_circle, true);
        a.recycle();
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setStrokeWidth(SECOND_LINE_WIDTH);
        secondCanvas.drawCircle(centerX, centerY, HOUR_LINE_WIDTH, paint);
