...
public class StickerView extends FrameLayout {
    ...
    //初始位置
    private ArrayList<Integer> initX = new ArrayList<>();
    private ArrayList<Integer> initY = new ArrayList<>();
    private ArrayList<Float> zoom = new ArrayList<>();
    ...
    public boolean replace(@Nullable Sticker sticker, boolean needStayState) {
        if (handlingSticker != null && sticker != null) {
            ...
            if (needStayState) {
                ...
                //  sticker.setId(handlingSticker.getId());
    ...
    @NonNull
    public StickerView addSticker(@NonNull Sticker sticker, int x, int y, float z) {
        initX.add(x);
        initY.add(y);
        zoom.add(z);
    ...
    @NonNull
    public StickerView addSticker(@NonNull Sticker sticker, float z) {
        initX.add(0);
        initY.add(0);
        zoom.add(z);
        return addSticker(sticker, Sticker.Position.CENTER);
...
//    private int i = 0;
//
//    private void setId(Sticker sticker) {
//        String s = null;
//        try {
//            TextSticker textSticker = (TextSticker) sticker;
//            s = textSticker.getText();
//        } catch (Exception ex) {
//            ex.printStackTrace();
//        }
//
...
//    }
    ...
    protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
        ...
        //   if (initBoolean) {
        if (initX.get(init) == 0 && initY.get(init) == 0) {
            sticker.getMatrix().postTranslate(offsetX, offsetY);
        } else {
            if (initX.get(init) == 0) {
                sticker.getMatrix().postTranslate(offsetX, initY.get(init));
            } else if (initY.get(init) == 0) {
                sticker.getMatrix().postTranslate(initX.get(init), offsetY);
            } else {
                sticker.getMatrix().postTranslate(initX.get(init), initY.get(init));
            }
